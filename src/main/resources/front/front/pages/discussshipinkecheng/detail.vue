...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscussshipinkechengCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscussshipinkechengCleanType')
				this.mescroll.num = 1
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discussshipinkecheng');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','discussshipinkecheng');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('discussshipinkecheng', this.id);
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscussshipinkecheng', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussdiscussshipinkecheng/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('discussshipinkecheng', this.detail);
				this.$utils.msg('审核成功');

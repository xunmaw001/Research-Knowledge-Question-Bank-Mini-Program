...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','kemufenlei');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','kemufenlei');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('kemufenlei', this.id);
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discusskemufenlei/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('kemufenlei', this.detail);
				this.$utils.msg('审核成功');

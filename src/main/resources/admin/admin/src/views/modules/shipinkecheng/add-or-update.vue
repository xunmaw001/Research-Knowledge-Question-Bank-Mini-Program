    ...
    >
      ...
      <el-col :span="24">  
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="解析" prop="jiexi">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.jiexi" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.jiexi" label="解析" prop="jiexi">
                    <span v-html="ruleForm.jiexi"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
  ...
  methods: {
    ...
    init(id,type) {
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          var json = data.data;
		if(json.jiangshigonghao!=''&&json.jiangshigonghao){
                this.ruleForm.jiangshigonghao = json.jiangshigonghao
	    		this.ro.jiangshigonghao = true;
		}
		if(json.jiangshixingming!=''&&json.jiangshixingming){
                this.ruleForm.jiangshixingming = json.jiangshixingming
	    		this.ro.jiangshixingming = true;
		}
        } else {
	...
	this.ruleForm.jiexi = this.ruleForm.jiexi.replace(reg,'../../../springboottqt04/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `shipinkecheng/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.shipinkechengCrossAddOrUpdateFlag = false;
						 this.parent.search();
